...
\begin{table}[h!]
    ...
    \begin{tabular}{|l|l|l|l|}
        \hline
        \textbf{Herramienta} & \textbf{Función} & \textbf{Ventajas} & \textbf{Limitaciones} \\
        \hline
        OpenTelemetry & Instrumentación & Estándar abierto, integración con múltiples backends & Curva de aprendizaje inicial \\
        \hline
        Prometheus & Métricas & Alta eficiencia, soporte Kubernetes & No centraliza logs ni trazas \\
        \hline
        Grafana & Visualización & Dashboards flexibles, soporte multifuente & Necesita otras herramientas para trazas/logs \\
        \hline
        Jaeger & Trazabilidad & Visualización de trazas distribuidas & Depende de la correcta instrumentación \\
        \hline
    \end{tabular}
    \vspace{0.2cm}
    \footnotesize{Fuente: elaboración propia a partir de documentación oficial de las herramientas.}
\end{table}
...
\paragraph{Automatización y CI/CD}\mbox{}

El despliegue inicial se realizó en Minikube de forma manual, pero el prototipo ya cuenta con automatización implementada mediante herramientas DevOps:

\begin{itemize}
    \item \textbf{Helm:} empaqueta todos los recursos de Kubernetes (Deployments, Services, ConfigMaps) en un solo \texttt{chart}, permitiendo despliegues reproducibles con un solo comando.
    \item \textbf{Ansible:} se utiliza para preparar el entorno local, instalar dependencias y ejecutar despliegues de Helm de manera automatizada.
    \item \textbf{CI/CD local:} mediante GitHub Actions, se ejecutan pruebas, se construyen imágenes Docker y se despliega automáticamente sobre Minikube, demostrando la viabilidad del flujo de integración y entrega continua.
\end{itemize}

Este enfoque asegura consistencia entre entornos, reduce errores humanos y documenta todos los pasos necesarios para reproducir la plataforma de forma confiable.

\paragraph{Reflexión y limitaciones}\mbox{}

El entorno local con Minikube permite experimentar y validar la instrumentación, visualización y trazabilidad. La automatización con Helm y Ansible ya se encuentra configurada y probada, aunque el entorno sigue presentando algunas limitaciones típicas de un clúster local:

\begin{itemize}
    \item No refleja completamente la escalabilidad ni la tolerancia a fallos de un clúster de producción en la nube.
    \item Algunos recursos, como almacenamiento persistente y networking avanzado, deben simularse o adaptarse.
\end{itemize}

El prototipo proporciona así una base sólida para futuras ampliaciones, incluyendo despliegues en entornos en la nube, integración de alertas avanzadas y análisis predictivo de métricas y trazas.
...
Oportunidades de mejora y ampliación incluyen:

\begin{itemize}
    \item Integración de un sistema de alertas automáticas.
    ...
    \item Ajustes y validación final de despliegues automatizados con Helm y Ansible.
\end{itemize}
