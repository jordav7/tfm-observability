...
\begin{table}[H]
  \centering
  \caption{Comparativa de tecnologías de observabilidad. Fuente: elaboración propia.}
...
\begin{table}[H]
  \centering
  \caption{Resumen de requisitos funcionales y no funcionales del prototipo de observabilidad. Fuente: elaboración propia.}
...
El prototipo integra desde esta entrega la automatización del despliegue y la gestión del entorno mediante herramientas DevOps:

\begin{itemize}
    \item \textbf{Helm:} Todos los recursos de Kubernetes (Deployments, Services, ConfigMaps) están empaquetados en un \texttt{chart} Helm, lo que permite desplegar el sistema completo con un solo comando, asegurando consistencia y reproducibilidad.
    \item \textbf{Ansible:} Se utiliza para preparar el entorno local, instalar dependencias necesarias y ejecutar los despliegues de Helm de manera automatizada, reduciendo errores manuales y facilitando la replicación.
    \item \textbf{CI/CD local:} Se ha configurado un flujo básico de integración continua que permite construir imágenes Docker y desplegar automáticamente sobre Minikube, demostrando la viabilidad del ciclo de integración y entrega continua.
\end{itemize}

Este enfoque no solo mejora la eficiencia, sino que también asegura consistencia entre entornos y mantiene documentación actualizada de todos los pasos necesarios para reproducir la plataforma.
...
\begin{itemize}
    ...
    \item La integración de Helm y Ansible ya está preparada y estructurada, pero su uso completo en entornos más grandes podría requerir ajustes adicionales.
\end{itemize}

Gracias a esta metodología, el prototipo proporciona una base sólida para futuras ampliaciones, incluyendo despliegues en entornos en la nube, integración de alertas avanzadas y análisis predictivo de métricas y trazas.
